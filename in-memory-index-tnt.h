...
/*
 * Migration runs under tnt_maintenance_lock(). Both nodes must be internal:
 * the history parent receives {child, parent}, and the child is emptied.
 * Fresh slabs replace both old slabs without changing either tree's links.
 */
enum { TNT_MIGRATE_DONE = 0, TNT_MIGRATE_NOOP = 1 };
